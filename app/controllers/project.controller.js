...
exports.updateProjectProgress = (req, res) => {
    const id = req.params.idProjectProgress;
...
// delete projectProgressLog
exports.deleteProjectProgressLog = async (req, res) => {
    const id = req.params.id;

    const deleLog = await ProjectProgressLog.destroy({ where: { id: id } });
    if (deleLog) {
        return res.status(200).send({ message: "Xoá Log thành công" });
    }
    res.status(500).send({ message: "Xoá Log thất bại" });
}
